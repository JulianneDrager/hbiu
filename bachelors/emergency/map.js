document.addEventListener("DOMContentLoaded", function () {
  ...
  function generatePageHTML(pageData, pageReq) {
    return `
      ...
      <div class="row">
        <div class="col-xl-8 col-lg-8">
          <div class="course-details">
            ...
            <hr>
            <p class="mb-20">${pageData.description}</p>
            <h3><i>Course Outline</i></h3>
            <hr>
            <!-- Display course outline in a structured format -->
            <ul>
              ${pageData.courseOutline
                .slice(0, 8) // Get the first 8 items
                .map(
                  (item) =>
            ...
            <br />
            <!-- Displays the next set of course outline items -->
            <ul>
              ${pageData.courseOutline
                .slice(8, 16) // Get the next 8 items
                .map(
                  (item) =>
                    `<li style="list-style-type: disc" class="ms-3">${item}</li>`
                )
            ...
            <ul>
              ${pageData.courseOutline
                .slice(16, 24) // Get the first 8 items
                .map(
                  (item) =>
                    `<li style="list-style-type: disc" class="ms-3">${item}</li>`
                )
            ...
            <ul>
              ${pageData.courseOutline
                .slice(24, 31) // Get the first 8 items
                .map(
                  (item) =>
                    `<li style="list-style-type: disc" class="ms-3">${item}</li>`
                )
