document.addEventListener("DOMContentLoaded", () => {
  ...
  galleryAWData[0].items.forEach((item, index) => {
  ...
  function showImage(index) {
    const item = galleryAWData[0].items[index];
  ...
  prevBtn.addEventListener("click", () => {
    currentIndex =
      (currentIndex - 1 + galleryAWData[0].items.length) %
      galleryAWData[0].items.length;
    showImage(currentIndex);
  });

  nextBtn.addEventListener("click", () => {
    currentIndex = (currentIndex + 1) % galleryAWData[0].items.length;
    showImage(currentIndex);
  });
});
